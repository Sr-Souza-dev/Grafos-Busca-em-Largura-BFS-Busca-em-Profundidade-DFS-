...
int main()
{
	...
	string linha;
	string intermediate;
	Graph graph = GraphInitialize(100,154);

	arquivo.open("../Arquivos(txt)/incidencia.txt");
	if(arquivo.is_open())
	{
		while(getline(arquivo,linha))
		{
			...
			if(int(tokens.size()) == 3)
				GraphInsertEdge(graph, stoi(tokens[0]), stoi(tokens[1]), stoi(tokens[2]));
			else	
				cout<<"[ERRO] Problemas na estrutura do arquivo!"<<endl;
			tokens.clear();
		}
	}
    printGraph(graph);
